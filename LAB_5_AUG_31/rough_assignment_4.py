    ...
    if depth > 0 and len(node.X) > 1 and node.entropy > entropy_threshold:  # Check depth, data samples, and entropy
        ...
        if best_feature is not None:                                   # Checks if a feature for splitting was identified 
            ...
            for value in unique_values:
                ...
                child = TreeNode(child_df, child_y, value, best_feature) # Creates a new tree node with this subset
    ...
    if depth > 0 and len(node.X) > 1 and node.entropy > entropy_threshold:  # Check depth, data samples, and entropy
        ...
        if best_feature is not None:
            ...
            for value in unique_values:
                ...
                Dec_Tree_Mod_stepwise(child, depth-1, entropy_threshold)  # Recursively calls Dec_Tree_Mod_stepwise on the child node, decreasing the depth by 1
    else: 
        return
    ...
    if depth > 0 and len(node.X) > 1 and node.entropy > entropy_threshold:  # Check depth, data samples, and entropy
        ...
        if best_feature is not None:
            ...
            for value in unique_values:
                ...
                Dec_Tree_Mod_early_stopping(child, depth-1, entropy_threshold, prev_val_accuracy) # Recursively calls Dec_Tree_Mod_early_stopping on the child node, decreasing the depth by 1
    else:
        return
