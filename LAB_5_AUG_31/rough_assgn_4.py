...
class TreeNode:
    ...
    def get_rules(self, path=[]):             # Recursive function to generate rules from the tree
        if not self.children:                 # Leaf node
            ...
            consequent = self.y.value_counts().idxmax()
            return [f"IF {antecedent} THEN {consequent}"]

        # If not leaf node, traverse its children
        rules_list = []
        for child in self.children:
            rules_list.extend(child.get_rules(path + [(self.feature, child.thresh)]))
...
# Implementing Dec_Tree_Mod to build the Decision Tree
def Dec_Tree_Mod(node, depth, entropy_threshold):
    if depth > 0 and len(node.X) > 1 and node.entropy > entropy_threshold:  # Check depth, data samples, and entropy
        ...
        for feature in node.X.columns:                                                         # No need to exclude the last column now as we aren't storing 'Acceptability' in X
            groups = [grp for _, grp in pd.concat([node.X, node.y], axis=1).groupby(feature)]  # Group by feature values and get associated 'Acceptability' values
        ...
        if best_feature is not None:
            ...
            for value in unique_values:
                child_df = node.X[node.X[best_feature] == value]
                child_y = node.y[node.X[best_feature] == value]
                child = TreeNode(child_df, child_y, value, best_feature)
                node.add_child(child)
                Dec_Tree_Mod(child, depth-1, entropy_threshold)
    else: 
        return
...
def predict_single(node, x):                    
    ...
    for child in node.children:                 # If not a leaf node, find the right child node to move
        if child.thresh == x[node.feature]:
...
def Dec_Tree_Mod_stepwise(node, depth, entropy_threshold):
    if depth > 0 and len(node.X) > 1 and node.entropy > entropy_threshold:  # Check depth, data samples, and entropy
        best_feature = None
        best_groups = None
        best_gain = -float('inf')
        ...
        if best_feature is not None:
            ...
            for value in unique_values:
                ...
                training_accuracies_after_each_branch.append(accuracy(y_train, predict(node, X_train)))
                validation_accuracies_after_each_branch.append(accuracy(y_validation, predict(node, X_validation)))

                Dec_Tree_Mod_stepwise(child, depth-1, entropy_threshold)
    else: 
        return
    
# Build the tree stepwise
root_stepwise = TreeNode(X_train, y_train, None, None)
Dec_Tree_Mod_stepwise(root_stepwise, 10, best_threshold_validation)
...
print(f"[EXPT - 2] PART - B : Validation accuracy Accuracy after each branch: ")
...
def Dec_Tree_Mod_early_stopping(node, depth, entropy_threshold, prev_val_accuracy=0.0):
    if depth > 0 and len(node.X) > 1 and node.entropy > entropy_threshold:
        best_feature = None
        best_groups = None
        best_gain = -float('inf')
        for feature in node.X.columns:  # No need to exclude the last column now as we aren't storing 'Acceptability' in X
        ...
        if best_feature is not None:
            ...
            for value in unique_values:
                ...
                val_accuracy = accuracy(y_validation, predict(node, X_validation))
                
                if val_accuracy < prev_val_accuracy: # Stop early if validation accuracy decreases
                    return
                
                prev_val_accuracy = val_accuracy
                Dec_Tree_Mod_early_stopping(child, depth-1, entropy_threshold, prev_val_accuracy)
    else:
        return
...
Dec_Tree_Mod_early_stopping(root_early_stopping, 10, best_threshold_validation)
...
sys.setrecursionlimit(10000)  # sets the recursion limit to 2000
...
if root_best_threshold.has_circular_reference():
    print("This Decision tree has a circular reference.")
else:
    print("This Decision tree does not have a circular reference.")

if root_best_threshold.check_parent_pointer():
    print("This Decision tree has a parent pointer inconsistency.")
else:
    print("This Decision tree's parent pointers are consistent.")

print("Rules for classification for the Decision Tree in Experiment 1 :")
root_best_threshold.print_rules()


print("\nRules for classification for the Decision Tree in Experiment 2 :")
root_stepwise.print_rules()
