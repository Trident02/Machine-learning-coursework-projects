...
# def information_gain(data, left, right, target):
#     original_entropy = entropy(data[target])
    
#     # expected average of the entropy of the left and right child nodes
#     expected_entropy = (len(left) / len(data)) * entropy(left[target]) + (len(right) / len(data)) * entropy(right[target])
...
def best_split(dataset, features, target, entropy_threshold):
    ...
    for feature in features:
        ...
        ig = entropy(dataset[target]) - expected_entropy  # Information gain = Entropy of target - Expected Entropy of the features
...
class TreeNode:
    ...
    def __init__(self, data, features, target, entropy_threshold):
        self.data = data                                      # the dataset associated with this node
    ...
    def build_child(self):
        ...
        self.split_feature = best_split(self.data, self.features, self.target, self.entropy_threshold)
        ...
        if not self.split_feature:
            ...
            unique_values = np.unique(self.data[self.split_feature])
            for value in unique_values:  
                child_data = self.data[self.data[self.split_feature] == value]                         # Filter the data of the current node based on the feature's unique value
                child_node = TreeNode(child_data, self.features, self.target, self.entropy_threshold)   # Create a new TreeNode (child node) using the filtered data               
