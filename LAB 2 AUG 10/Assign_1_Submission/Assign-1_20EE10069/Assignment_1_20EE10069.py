...
# IMP NOTE - 1 : Due to implementation of "PERCEPTRON" instead of KNeighboursClassifier for K-Fold Cross Validation using 2000 iterations, the code is running little slow (may atke 2-3 mins for complete execution).
# IMP NOTE - 2 : During the execution of code, the plots will show up, Kindly close the plots for continuing the execution of code.


# importing essential libraries
import numpy as np                                 # Import the numpy library to work with arrays in Python
import matplotlib.pyplot as plt                    # Import matplotlib's pyplot, a plotting library

from sklearn.model_selection import KFold          # Import KFold from scikit-learn to perform K-Fold Cross-Validation
...
# Implementation of PLA  (Perceptron Learning Algorithm) for DataSet - 1 :
...
class Perceptron:
    def __init__(self, MAX_iters) :
        self.MAX_iters = MAX_iters                          # Constructor: Initialize maximum number of iterations for training the perceptron
        
    def predict(self, inputs):                              # Make a prediction based on the current weights and input features
    ...
    def fit(self, X, y):
        ...
        for _ in range(self.MAX_iters):
            ...
            for inputs, y_label in zip(X, y):
                ...
                if y_prediction != y_label:                      # Update the weights if there is a misclassification
...
# Creating a list of different K_values for implementing K Fold Cross Validation
K_values = [2, 4, 7, 8]
...
for k in K_values:
    ...
    for train_index, test_index in kf.split(X_1):      # this loop will run for 'k' times
        ...
        metrics = PerformanceMetrics(y_test, prediction)   # creating an object for the "PerfomanceMetrics" to compute Performance Metrics class
    ...
    # Calculating the mean of the metrics for K manually
    ...
    # Calculating the variance of the metrics for K manually
    ...
    if k != 8 :
        print("\nKindly PLEASE WAIT ! The performance metrics are being computed.")
    else : 
        print("\nKindly PLEASE WAIT ! The plot will show up !")
...
# As suggested by the Prof. Plaban Kumar Bhowmick, the "train_test_split" library can be used without penalty.
...
metrics_train_2a = PerformanceMetrics(y_train_2a, y_pred_train_2a)                # creating an object for the "PerfomanceMetrics" class
...
metrics_train_3a = PerformanceMetrics(y_train_3a, y_pred_train_3a)                 # creating an object for the "PerfomanceMetrics" class
...
print("\nThank you ! That's all for the MLFA ASSIGNMENT - 1. Please go through the PDF file for data, plots and observations !")
